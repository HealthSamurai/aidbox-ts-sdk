...
export function wrapCache(opts: ServerOptions): ServerOptions {
	...
	const resolve = async (canonicalUrl: string): Promise<Resource | null> => {
		let value: Resource | null | undefined;
		try {
			value = await tryCache<Resource | null>(
				canonicalByUrlStore,
				canonicalUrl,
			);
		} catch (e) {
		...
		value = await opts.resolve(canonicalUrl);
	...
	const search = async (
		...
		let value: Resource[] | null | undefined;
		try {
			value = await tryCache<Resource[] | null>(canonicalsByKindStore, kind);
		} catch (e) {
			console.error(e);
